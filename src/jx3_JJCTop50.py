...
class GetJJCTop50Record:
    ...
    async def get_top50_history(self, typeName: str, heiMaBang: bool):
        # 准备请求参数
        ...
        for element in data.get("data")[:50]:
            ...
            if school in jxData.much_school:
                ...
                if kungfu in jxData.school_pinyin:
                    value = await jxData.school_pinyin[kungfu]
                    school_top[value] = school_top.get(value, 0) + 1
            else:
    ...
    async def main(self):
        # 获取所有的数据进行处理
        data = await self.get_top50_history('week', False)
        print(data)
        # 判断连接池数据是否冲突
        ...
        sql = "insert into JJC_rank50_weekly (week, 霸刀, 藏剑, 蓬莱, 无方,云裳,花间,少林,惊羽,丐帮,苍云,紫霞,相知,补天,凌雪,明教,毒经,灵素,天策,田螺,胎虚,离经,莫问,衍天,冰心) values ('%s','%s','%s',%s,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (self.weekly, data["霸刀"], data["藏剑"], data["蓬莱"], data["无方"], data["云裳"], data["花间"], data["少林"],data["惊羽"],data["丐帮"],data["苍云"], data["紫霞"], data["相知"], data["补天"], data["凌雪阁"], data["明教"], data["毒经"], data["灵素"],data["天策"],data["田螺"], data["胎虚"], data["离经"], data["莫问"], data["衍天宗"], data["冰心"])
        print(sql)
...
getJJCTopRecord = GetJJCTop50Record(33)
